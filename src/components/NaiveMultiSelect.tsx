...
function multiSelectValue({
    ...
    return(
        <>
        <div {...others}>
            <div className = "flex items-center justify-between rounded-md p-1" style={divStyle}>
                <span>
                    {label}
                </span>
                <CloseButton 
...
const multiSelectItem = forwardRef<HTMLDivElement,itemProps>(({label,bgColor,textColor,...others}, ref) => {
  const divStyle = {
    ...
    return(
        <div {...others} ref={ref} >
        <div className = "flex items-center justify-between rounded-md p-1 w-min" style={divStyle}>
            <span>{label}</span>
        </div>
        </div>
    )
